...
func TestEndpointService_TamperProtection(t *testing.T) {
	...
	mux.HandleFunc("/endpoint/v1/endpoints/endpointid-guid/tamper-protection", func(w http.ResponseWriter, r *http.Request) {
		testMethod(t, r, "GET")
		w.Header().Set("Content-Type", "application/json")
		w.Write([]byte(`{
  "password": "password",
  ...
  "enabled": true
	...
	got, _, err := client.Endpoints.TamperProtection(ctx, "tenantid-guid", "", "endpointid-guid")
	if err != nil {
		assert.NoErrorf(t, err, "EndpointService.GetTamperProtection returned error: %v", err)
	}
	assert.NotNil(t, got)
